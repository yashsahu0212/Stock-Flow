...
class DashboardService {
    static async getSummary() {
        ...
        const inventorySum = await db_1.default.inventory.aggregate({
            _sum: {
                quantity: true,
            },
        });
        ...
        const allProducts = await db_1.default.product.findMany({
            include: {
                inventories: {
        ...
        const pendingOrders = await db_1.default.order.count({
            where: {
                status: {
                    in: [client_1.OrderStatus.PENDING, client_1.OrderStatus.PICKING],
                },
            },
        });
        ...
        // 5. Today's Movements
        const todayMovements = await db_1.default.stockMovement.count({
            where: {
                createdAt: { gte: todayStart },
            },
        });
        // 6. Today's Picks
        const todayPicks = await db_1.default.stockMovement.count({
            where: {
